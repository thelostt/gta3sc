...
{
    visit_one(this->compiled, [this](const auto& h) {
        this->bw = BinaryWriter(h.compiled_size());
        generate_code(h, *this);
    });
}
...
inline size_t CustomHeaderOATC::compiled_size() const
{
    size_t size = 12 + 4 + 2 + 2 + (12 * this->ordinal_commands.size());
    for(auto& pair : ordinal_commands) size += pair.first->name.size() + 1;
    return size;
}
...
{
    return x.generate_code(codegen);
}
...
inline void generate_code(const shared_ptr<Label>& label_ptr, CodeGenerator& codegen)
{
    ...
    else // current script is mission/stream
    {
        ...
        else // label is within main block
        {
            if(codegen.program.opt.use_local_offsets)
                codegen.program.error(*codegen.script, "cannot branch from this script into main block using local offsets [-mlocal-offsets]");
...
static void generate_header_start(const CustomHeader& header, CodeGeneratorData& codegen)
{
    generate_skipper(codegen, header.compiled_size(), false);
    codegen.bw.emplace_u8(0xFF);
    codegen.bw.emplace_u8(0x7F);
    codegen.bw.emplace_u8(0xFE);
    codegen.bw.emplace_u16(0x0000);
}
...
inline void generate_code(const CompiledScmHeader& header, CodeGeneratorData& codegen)
{
    ...
    {
        return generate_skipper(codegen, 8 + skip_bytes, true);
    };
...
inline void CustomHeaderOATC::generate_code(CodeGeneratorData& codegen) const
{
    generate_header_start(*this, codegen);
    codegen.bw.emplace_bytes(4, "OATC");

    // relative to OATC
    size_t next_name_offset = 4 + 2 + 2 + (12 * this->ordinal_commands.size());

    codegen.bw.emplace_u16(this->starting_opcode);
    codegen.bw.emplace_u16((uint16_t)this->ordinal_commands.size());

    for(auto& pair : ordinal_commands)
    {
        codegen.bw.emplace_u32(0);
        codegen.bw.emplace_u32(pair.first->hash.value());
        codegen.bw.emplace_u32(next_name_offset);
        next_name_offset += pair.first->name.size() + 1;
    }

    for(auto& pair : ordinal_commands)
    {
        auto& name = pair.first->name;
        codegen.bw.emplace_bytes(name.size() + 1, name.c_str());
    }
}
